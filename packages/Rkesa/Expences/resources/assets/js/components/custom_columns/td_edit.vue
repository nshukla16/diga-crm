<template lang="pug">
    div
        router-link.btn.btn-secondary(style="margin-right: 5px", :to="{name: 'expences_edit', params: {id: row.id}}") {{ $t('service.Service_edit') }}
    ...
    methods: {
        remove_manufacturer(id) {
            if (confirm(this.$root.$t('estimate.Are_you_sure_want_to_delete_this'))) {
                this.$http.delete('/api/expences/' + id).then(res => {
