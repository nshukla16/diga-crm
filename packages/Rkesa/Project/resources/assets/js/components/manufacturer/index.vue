<template lang="pug">
    div
        h2 {{ $t('project.Manufacturers') }}
        section.diga-container.p-4
            div.float-sm-right.mr-2
                router-link.btn.btn-diga(style="height:38px;", :to="{name: 'manufacturer_create'}") {{ $t('project.Manufacturer_new') }}
...
export default {
    props: ['manufacturer', 'offset'],
    data(){
        return {
            my_manufacturer: this.manufacturer,
            table: {
                columns: [
                    { title: '#', field: 'id', sortable: true },
                    { title: this.$root.$t('project.Manufacturer_name'), field: 'name', tdComp: name_column, sortable: true },
    ...
    mounted(){
        document.title = this.$root.global_settings.site_name + ' | ' + this.$root.$t('project.Manufacturers');
    ...
    methods: {
        getResults() {
            this.$http.get('/api/manufacturers?' + this.$root.params(this.table.query)).then(res => {
                return res.json();
            }).then(data => {
                if (data.errcode == 1) {
