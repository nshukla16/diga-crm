<template lang="pug">
    div
        router-link.btn.btn-secondary(style="margin-right: 5px", :to="{name: 'equipment_edit', params: {id: row.id}}") {{ $t('service.Service_edit') }}
...
export default {
    ...
    methods: {
        remove_manufacturer(id) {
            if (confirm(this.$root.$t('project.Are_you_sure_you_want_to_delete_equipment'))) {
                // let url = this.$root.contact_or_client_show(id);
                this.$http.delete('/api/equipments/' + id).then(res => {
