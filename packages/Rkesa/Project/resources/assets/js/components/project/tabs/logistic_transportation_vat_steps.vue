...
<template lang="pug">
    div.mb-2
        div.project-section {{ $t('project.Transportation_vat_expenses_block') }}
        table.contract_payment_steps.table.table-striped
            ...
            tbody
                tr(v-for="(payment,i) in order.transportation_vat_payments")
                    td
...
export default {
    ...
    created(){
        if (this.order.transportation_vat_payments.length == 0){
            this.add_step();
        }
    },
    computed: {
        ...
        total_price_in_main_currency(){
            let total_price = this.order.transportation_vat_payments.map(p => parseFloat(p.in_main_currency)).reduce((a, b) => a + b, 0).toFixed(2);
            this.order.transportation_vat_total = total_price;
            return total_price;
        },
    },
    methods: {
        ...
        add_step(){
            ...
            this.order.transportation_vat_payments.push(transPayment);
        },
        remove_step(payment){
            if (confirm(this.$root.$t("project.Sure_remove_step"))){
                if (!('removed_transportation_vat_payments' in this.order)){
                    this.order.removed_transportation_vat_payments = [];
                }
                this.order.removed_transportation_vat_payments.push(payment.id);
                let index = this.order.transportation_vat_payments.indexOf(payment);
                this.order.transportation_vat_payments.splice(index, 1);
            }
        },
    },
}
