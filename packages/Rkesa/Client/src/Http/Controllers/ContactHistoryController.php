...
class ContactHistoryController extends Controller
{
    public function index(Request $request, $id)
    {
        ...
        try {
            ...
            $histories = ClientHistory::with(['user', 'service', 'service_attachment', 'event.event_type', 'client_contact', 'call'])->where('client_contact_id', $id);
    ...
    public function message(Request $request, $id)
    {
        ...
        $res->errcode = 0;
        try {
            $client_history = ClientHistory::find($id);
            if ($client_history) {
                $res->message = $client_history->message;
            } else {
                $res->errcode = 1;
            }
        } catch (Exception $e) {
