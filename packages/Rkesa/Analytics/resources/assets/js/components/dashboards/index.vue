...
section.dashboard-wrapper
  ...
  div(v-else)
    ...
    section.stage-widgets
      ...
      draggable.row(v-model="widgets")
        stage-widget(
          ...
          :key="widget.id",
          :widget="widget",
          v-on:popup_table="popup_table",
          v-on:popup_line="popup_line"
        )
    #modal-full-list-table.modal.fade(tabindex="-1", role="dialog")
      .modal-dialog
        .modal-content(v-if="popup_rows != null")
          .modal-header
    ...
    #modal-full-list-line.modal.fade(tabindex="-1", role="dialog")
      .modal-dialog
        .modal-content(v-if="popup_rows_for_line != null")
          .modal-header
              ...
              table.table.table-bordered.table-striped
                ...
                tbody
                  tr(v-for="(row, index) in popup_rows_for_line")
                    ...
                    td
                      button.btn.btn-diga(
                        v-on:click="open_client_page_through_line(row.client_contact_id)"
                      ) >>
          .modal-footer
            div(style="float: left")
              span(v-text="$t('dashboard.quantity') + ': '")
...
import stageStore from "./../../../../../../Dashboard/resources/assets/js/vuex/modules/stage.js";
import widgetsHeader from "./../../../../../../Dashboard/resources/assets/js/components/dashboard/widgets_header.vue";
import stageWidget from "./../../../../../../Dashboard/resources/assets/js/components/dashboard/widget.stage.vue";
import tableTable from "./../../../../../../Dashboard/resources/assets/js/components/dashboard/charts/table-table.stage.vue";
...
export default {
  ...
  methods: {
    ...
    load_config() {
      ...
      return new Promise((resolve, reject) => {
        this.$http.get("/api/dashboards/config").then(
          (res) => {
  ...
  created() {
    ...
    document.title =
      ...
      " | " +
      this.$root.$t("dashboard.Dashboard");
  },
  beforeDestroy() {
  ...
  computed: {
    master_sum_index() {
      return this.$store.getters["stage/master_sum_index"](
        this.current_entity.service_state_id
      );
    },
    ordered_entities() {
      return Object.values(this.$store.getters["stage/entities"]).sort(
        function (a, b) {
    ...
    widgets: {
      ...
      set(value) {
        ...
        this.$http
          .post("/api/hr/" + this.$root.user.id + "/dashboard_widgets", {
            dashboard_widget_ids: ids,
          })
          .then(
            (res) => {
              if (res.data.errcode == 1) {
